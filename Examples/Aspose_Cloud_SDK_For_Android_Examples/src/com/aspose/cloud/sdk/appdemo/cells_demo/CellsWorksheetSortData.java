...
public class CellsWorksheetSortData extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cells_worksheet_sortdata);
		init();
			...
			public void onClick(View arg0) {
				...
				if (function_arg2.getText().length() == 0
					...
					AlertDialog.Builder dialog = new AlertDialog.Builder(
							CellsWorksheetSortData.this);
					dialog.setTitle("Error");
					dialog.setMessage("Please Enter Require Fields");
					dialog.setNeutralButton("Ok", null);
					dialog.show();
				} else {
					cellArea = function_arg2.getText().toString();
					dataSort = new DataSort();
					Worksheet obj = new Worksheet(constructor_arg1.getText()
							.toString(), constructor_arg2.getText().toString());
					response = obj.sortData(dataSort, cellArea);
					if (response) {
						result.append("Data Sorted Successfully");
					} else {
						result.append("Oopps...Something went wrong");
					}
				}
			}
		});
	...
	private void init() {
		...
		function_arg2 = (EditText) findViewById(R.id.cells_worksheet_sortdata_arg1);
