...
public class PdfAnnotationEditorGetAllLinks extends Activity {
	private EditText constructor_arg1;
	private EditText function_arg1;
	private TextView result;
	private Button btnSubmit;
	protected int pageNumber;
	protected List<Link> response;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.pdf_annotationeditor_getalllinks);
		init();
			...
			public void onClick(View arg0) {
				...
				if (function_arg1.getText().length() == 0
					...
					AlertDialog.Builder dialog = new AlertDialog.Builder(
							PdfAnnotationEditorGetAllLinks.this);
					...
					response = obj.getAllLinks(pageNumber);
					if (response == null) {
						Toast.makeText(PdfAnnotationEditorGetAllLinks.this,
								"Server Response Null", Toast.LENGTH_LONG)
								.show();
						result.append("No Link Found");
					} else {
						result.append("\n List: \n");
						for (Link item : response) {
							result.append(item.getAction() + " \n");
						}
	...
	private void init() {
		...
		function_arg1 = (EditText) findViewById(R.id.pdf_annotationeditor_getalllinks_arg1);
