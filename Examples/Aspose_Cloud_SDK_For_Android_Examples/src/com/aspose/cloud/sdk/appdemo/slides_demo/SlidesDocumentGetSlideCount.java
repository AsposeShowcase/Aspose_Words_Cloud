...
public class SlidesDocumentGetSlideCount extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.slides_document_getslidecount);
		init();
			...
			public void onClick(View arg0) {
				...
				if (constructor_arg1.getText().length() == 0) {
					AlertDialog.Builder dialog = new AlertDialog.Builder(
							SlidesDocumentGetSlideCount.this);
					...
					Document obj = new Document(constructor_arg1.getText()
							.toString());
					response = obj.getSlideCount();
					if (response > 0) {
						result.append("Numbers Of Slides = " + response);
					} else {
						result.append("No Slide Found");
					}
				}
			}
		});
